...
const Shop = () => {
    ...
    const [handleSelect, , , selectedItem] = useSelectButton() 
    ...
    const { data: shopData = [] } = useQuery({
        queryKey: ['shop'],
        queryFn: async () => {
            const { data } = await axiosPublic.get('/shop',)
            return data;
        }
    })
    ...
    return (
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    {
                        shopData.map(item => <tr key={item._id} className="border-b border-gray-200 dark:border-gray-700">
                            ...
                            <td className="px-6 py-4 bg-gray-50 dark:bg-gray-800">
                                <button onClick={()=> handleSelect(item._id)} className={`${selectedItem === item._id ? 'py-1 px-3 rounded-md bg-green-500 text-white' : 'py-1 px-3 rounded-md bg-amber-800 text-white'}`} >Select</button>
                            </td>
                                    ...
                                    medicineDetails && <dialog id="my_modal_4" className="modal">
                                        <div className="modal-box w-11/12 max-w-5xl">
                                            <div className='flex items-center justify-center gap-6 px-20'>
                                                <div className='w-1/2'>
                                                    <img className='w-full h-full' src={medicineDetails.imageUrl} alt="" />
                                                </div>
